...
- **Owner-only Notebook deletion + Notebook tags** (2026-09-10, `js/app.js`): `initSwipeToDelete_()` (delegated Pointer Events on `document`) plus `confirmDeleteNotebookClient_()`/`confirmDeleteNotebookTransaction_()` handle swipe-to-delete for Notebook (Short) clients and their individual payment/invoice entries — owner-role-gated, never optimistic, paired with an explicit desktop fallback action everywhere it renders. `toggleClientTag_()`/`createAndAssignTag_()`/the "Manage tags" panel (`openTagsPanel()` and friends) handle Notebook tags — edit-role gated for mutation, open to everyone for viewing/filtering. See `ARCHITECTURE.md`'s matching section and `DECISIONS.md`'s 2026-09-10 entries for the full design.
